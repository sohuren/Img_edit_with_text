...
# the remaining thing is the same with previous model
class Pix2PixModel_Bucket2(BaseModel):
    def name(self):
        return 'Pix2PixModel_Bucket2'
	...
	self.pre_trained_bucket = opt.pre_trained_bucket
        ...
        self.netRNNBucket = networks.define_ENcoder(opt.embed_size, opt.hidden_size, opt.vocab_size, opt.visual_size, opt.num_layers, opt.rnn, self.wordvec, self.gpu_ids)                            
	# encoder for the image captioning
        ...
        if not self.isTrain or opt.continue_train:
            ...
            self.load_network(self.netRNNBucket, 'RNNBucket', opt.which_epoch)    
	...
	if self.isTrain:
	    Generator_params = list(self.netEn.parameters()) + list(self.netRNNBucket.parameters())	
	    ...
	    if opt.lambda_p != 0:
	       self.criterionMSE = torch.nn.MSELoss()	
            ...
            networks.print_network(self.netRNNBucket)
	...
	# this is the one used for descriptions for each bucket  	
	    ...
	    length = [description_bucket.size(1)]	   		
            visual_vec = self.netRNNBucket.forward(description_bucket, length)  #take the text associated with the bucket
    ...
    def test(self):
        ...
        for i in range(self.num_G):
            ...
            if torch.cuda.is_available():
               description_bucket = description_bucket.cuda()
            length = [description_bucket.size(1)]   		     	
            visual_vec = self.netRNNBucket.forward(description_bucket, length)
            self.fake_B_attention[i] = word_vec.dot(visual_vec)[None]
	...
	self.save_network(self.netRNNBucket, 'RNNBucket', label, use_gpu)
