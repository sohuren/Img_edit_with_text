...
# has the constrain that image size should be the same, say 512*512, we don't use it here now
# this code can be ignored, we just keep it here
class Pix2PixModel_Bucket(BaseModel):
    def name(self):
        return 'Pix2PixModel_Bucket'
	...
	# load the wordvec from pre-trained dataset 
	if opt.wordvec:
	   ...
	   self.wordvec = pickle.load(fp)
        ...
        for i in range(opt.num_G): 
            ...
            self.netMLP[i] = networks.define_ResnetMLP(opt.input_nc, opt.output_nc, opt.ngf, opt.norm, 3, opt.visual_size, self.gpu_ids)
	...
	Discrinator_params = list(self.netD.parameters()) + list(self.netEnDis.parameters())	
	    ...
	    if opt.lambda_p != 0:
	       self.criterionMSE = torch.nn.MSELoss()	
            ...
            for i in range(opt.num_G):
                networks.print_network(self.netMLP[i])
                networks.print_network(self.netG[i])
	...
	# we will ignore the bucket description in this model since we use the the visual information
	...
	self.real_A = Variable(self.input_A)
        ...
        for i in range(self.num_G):
            ...
            visual_vec = self.netMLP[i].forward(self.fake_B_bucket[i]-self.real_A) # take the delta image difference
	    visual_vec = torch.transpose(visual_vec, 0, 1)	
    ...
    def test(self):
        ...
        for i in range(self.num_G):
            self.fake_B_bucket[i] = self.netG[i].forward(self.real_A)
            visual_vec = self.netMLP[i].forward(self.fake_B_bucket[i]-self.real_A)
	    visual_vec = torch.transpose(visual_vec, 0, 1)	
            self.fake_B_attention[i] = word_vec.dot(visual_vec)[None]
	...
	# argmax or fusion, we need to add it		
	atts = torch.split(attention, 1, 1)		
	img_size = self.fake_B_bucket[0].size()
	self.fake_B = self.fake_B_bucket[0]*atts[0].expand(img_size)
	for i in range(1, self.num_G):  
	...
	if not self.triple:
	   ...
	   # real text
           ...
           self.loss_D = self.loss_D_fake*2/3.0 + self.loss_D_real * 0.333
           self.loss_D.backward() 
    ...
    def save(self, label):
        ...
        for i in range(self.num_G):
            self.save_network(self.netG[i], 'G'+str(i), label, use_gpu)
            self.save_network(self.netMLP[i], 'MLP'+str(i), label, use_gpu)
