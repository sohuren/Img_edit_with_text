...
# this model contains one lstm for the user input speech
# every other thing is the same previous model except we directly classify the text into one bucket and didn't care about the text associated with the bucket  

class Pix2PixModel_Bucket6(BaseModel):
    def name(self):
        return 'Pix2PixModel_Bucket5'
	...
	self.pre_trained_bucket = opt.pre_trained_bucket
        ...
        self.netG = networks.define_G_FilterBank(opt.input_nc, opt.output_nc, opt.ngf,
	                            opt.which_model_netG, opt.norm, opt.num_G, self.gpu_ids)

        # encoder for the image captioning, output a distribution over bucket 
        self.netEn = networks.define_ENcoder_Linear(opt.embed_size, opt.hidden_size, opt.vocab_size, opt.num_G, opt.num_layers, opt.rnn, self.wordvec, self.gpu_ids)
	...
	if self.isTrain:
	    Generator_params =  list(self.netG.parameters()) + list(self.netEn.parameters())
	...
	length = [descriptions.size()[1]]  
        fc = self.netEn.forward(descriptions, length)

	# this is the one used for descriptions for each bucket  	
        # now get the attetion weight for each different models	
        self.fake_B_bucket = self.netG.forward(self.real_A)

	attention = self.softmax(fc)
	...
	print attention[0].data
        ...
        # now get the attention weight for each different models	
        self.fake_B_bucket = self.netG.forward(self.real_A)
        print self.bucket     	
	# fusion with different weights or just take the argmax 
        self.fake_B = self.fake_B_bucket[self.bucket]
